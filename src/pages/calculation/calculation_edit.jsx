...
import {CosButton,CosInputText,CosIptNumber,CosSelect,CosDoubleDatePicker,CosPaginationTable} from '@/components/common/index'
...
const CommissionAgmtEdit = (props) => {
	...
	}, [dateEnd, tableData, addFlag, btnData, flag, headerUuid, commonFlag, btnIdx])
	...
	const saveMockData = async() => {
		...
		if(data) {
			...
			Toast('',formatMessage({id: 'lbl.Group-is-required'}), 'alert-warning', 5000, false);
		}
	...
	const addItem = () => {
		...
		if (len == 0) {
			...
			itemid ? addItemFlag() : Toast('',formatMessage({id: 'lbl.save-add-item'}), 'alert-warning', 5000, false);
	...
	const tableSave = async (record, index) => {
		...
		if(DataFlag) {
			Toast('',formatMessage({id: 'lbl.Please-enter-required-options'}), 'alert-warning', 5000, false)
		} else {
	...
	const deleteItem = async (text, record, index) => {
		...
		const confirmModal = confirm({
            ...
            content: formatMessage({id: 'lbl.delete-por'}) + record.porCountry + formatMessage({id: 'lbl.de-data'}),
			...
			render: (text, record, index) => {
				return <div>
					...
					<Tooltip title={<FormattedMessage id='btn.save' />}>
						{/* 保存 */}
						<a style={{display: record.saveShowHide ? 'inline-block' : 'none'}} disabled={commonFlag} onClick={() => tableSave(record, index)}><FileDoneOutlined /></a>
					</Tooltip>&nbsp;
	...
	const computingMethodColumns = [
		{
			...
			render: (text, record, index) => {
				return <div>
					...
					<Tooltip title={<FormattedMessage id='btn.save' />}>
						<a style={{display: record.saveShowHide ? 'inline-block' : 'none'}} disabled={commonFlag} onClick={() => boxTableSave(record, index)}><FileDoneOutlined /></a>
					</Tooltip>&nbsp;
					<Tooltip title={<FormattedMessage id='btn.edit' />}>
			...
			render: (text, record) => {
				return <div>
					{record.saveShowHide ? <Select defaultValue={record.unitPriceType} onChange={(e) => getCommonSelectVal(e, record, 'unitPriceType')} name='unitPriceType' options={calculationType} /> : text}
				</div>
			}
		}
	]
	...
	const boxTableSave = async (record, index) => {
		...
		if(!record.containerSizeTypeGroup) {
			...
			if(itemUuidFlag){
				...
				Toast('', formatMessage({id: 'lbl.save-add-item-box-detailed'}), 'alert-warning', 5000, false)
			}
	...
	const [itemUuidFlag, setItemUuidFlag] = useState(false);
	...
	const getCommonSelectVal = (e, record, name, flag) => {
		...
		// if(!flag && itemUuidFlag == uuids){
			console.log(itemUuidFlag,uuids, record)
	...
	const addItemDetailed = async () => {
		...
		if (len == 0) {
			...
			computingMethodData[len - 1].commissionContainerPriceUuid ? commonPrice() : Toast('', formatMessage({id: 'lbl.save-add-data'}), 'alert-warning', 5000, false);
		}
	...
	const rightBtn = () => {
		...
		if(data.yfBusiness) {
			...
			Toast('', formatMessage({id: 'lbl.Goods-category-required'}), 'alert-warning', 5000, false)
		}
	}
	...
	const saveBoxSize = async () => {
		...
		if(data.containerSizeTypeGroup && data.yfBusiness) {
			...
			Toast('', formatMessage({id: 'lbl.box-size-Goods-category-required'}), 'alert-warning', 5000, false)
		}
	}
		...
		<>
			<Modal title={title} visible={isModalVisible} footer={null} width="100%" height="100%" onCancel={() => handleCancel()} maskClosable={false}>
				<div className="topBox">
					<Form onFinish={handleQuery} form={queryForm}>
						<Row>
							...
							{/* 代理编码 */}
							<CosInputText span={6} disabled name='agencyCode' label={<FormattedMessage id='lbl.agency' />} capitalized={false}/>
							{/* 协议代码 */}
							<CosInputText span={6} disabled name='commissionAgreementCode' label={<FormattedMessage id='lbl.agreement' />} capitalized={false}/>
							{/* 开始日期 */}
							...
							{/* Cross Booking模式 */}
							{/* <CosSelect disabled={commonFlag} span={6} name='crossBookingMode' label={<FormattedMessage id='lbl.crosstype' />} options={pattern} /> */}
							{/* 第三地佣金付费模式 */}
							{/* <CosSelect disabled={commonFlag} span={6} name='payElsewhereMode' label={<FormattedMessage id='lbl.third' />} options={paidCommissionModel} /> */}
							{/* All in Rate */}
							...
							{/* 记账算法 */}
							{/* <CosSelect disabled={commonFlag} span={6} name='postCalculationFlag' label={<div><span style={{color: 'red'}}>*</span><FormattedMessage id='lbl.arithmetic' /></div>} options={accountsArithmetic} /> */}
							{/* 记账方式 */}
							{/* <CosSelect disabled={commonFlag} span={6} name='postMode' label={<div><span style={{color: 'red'}}>*</span><FormattedMessage id='lbl.bookkeeping' /></div>} options={accountsWay} /> */}
							{/* 向谁预估 */}
							<CosInputText disabled={commonFlag} maxLength={10} span={6} name='ygSide' label={<FormattedMessage id='lbl.estimate' />} capitalized={false}/>
							{/* 向谁开票 */}
							<CosInputText disabled={commonFlag} maxLength={10} span={6} name='yfSide' label={<FormattedMessage id='lbl.make' />} capitalized={false}/>
							{/* 向谁报账 */}
							<CosInputText disabled={commonFlag} maxLength={10} span={6} name='sfSide' label={<FormattedMessage id='lbl.submitanexpenseaccount' />} capitalized={false}/>
							{/* 预提是否记账 */}
							{/* <CosSelect disabled={commonFlag} span={6} name='isYt' label={<div><span style={{color: 'red'}}>*</span><FormattedMessage id='lbl.withholding' /></div>} options={ytBusiness} /> */}
							{/* 应付实付是否记账 */}
							{/* <CosSelect disabled={commonFlag} span={6} name='isBill' label={<div><span style={{color: 'red'}}>*</span><FormattedMessage id='lbl.actually' /></div>} options={yfBusiness} /> */}
						</Row>
					</Form>
				</div>
				<div className="more-btn" style={{display: cssNone ? 'block' : 'none'}}>
					{
						btnData.map((val, idx) => {
							return <CosButton key={idx}>{val.icon}{val.label}</CosButton>
						})
					}
				</div>
					...
					<Tabs onChange={callback} type="card" activeKey={defaultKey}>
						...
						<TabPane tab={<FormattedMessage id='lbl.group-message' />} key="2">
							<div style={{ width: '40%', border: '1px solid #aaaaaa', padding: '10px', display: 'inline-block', borderRadius: '10px' }}>
								<div><FormattedMessage id='lbl.maintain-na-message' /></div><br />
								...
								<div className='box-size-group-main'>
									<Form form={queryForm}>
										<Row className='box-size-group-main-input'>
											...
											{/* <CosSelect span={10} disabled={!stateFlags} name='yfBusiness' label={<FormattedMessage id='lbl.cargo-class' />} options={natureCode} /> */}
										</Row>
										<Row className='box-size-group-main-input'>
