...
const convertedRate = () => {
    ...
    //未转费率
    const [unTableDatas, setUnTableDatas] = useState([])
    const [unPage, setUnPage] = useState({ pageSize: 20, current: 1 })
    const [unTabTotal, setUnTabTotal] = useState(0)
    //已转费率
    ...
    const pageChange = async(pagination, options) => {
        ...
        const result =  await request($apiUrl.YET_LOCAL_CHARGE_REPORT_SEARCH_LIST,{
            method:'POST',
            data:{
                'page':{
                    ...pagination
    ...
    const deleteItem = (record, index,flag) => {//删除Item
        ...
        const confirmModal = confirm({
            ...
            async onOk() {
                ...
                setLoading(true)
                const result =  await request($apiUrl.YET_LOCAL_CHARGE_REPORT_DELETE,{
                    method:'POST',
                    data:{
                        paramsList:deteUuid
                    }
    ...
    const saveItem = async(record, index) => {//保存Item
        setLoading(true)
        setObjMessage(null)
        console.log(queryForm.getFieldsValue())
        delete record.show
        return
        let url = $apiUrl
        let column = queryForm.getFieldValue().unColumn
        if(defaultKey==2){
            url = $apiUrl
            column = queryForm.getFieldValue().column
        }
        const result =  await request(url,{
            ...
            data:{
                'params':{
                    ...record,
                    ...column,
                    fromDate:column.fromDate?momentFormat(column.fromDate):null,
                    toDate:column.toDate?momentFormat(column.toDate):null,
                }
            }
        })
        if(result.success){
            pageChange(defaultKey==1?unPage:page)
            setLoading(false)
        }else{
            setObjMessage({alertStatus: 'alert-error', message: result.errorMessage})
        }
    ...
    const save=async()=>{
        ...
        const result =  await request($apiUrl.YET_LOCAL_CHARGE_REPORT_SAVE_OR_UPDATE,{
            method:'POST',
            data:{
                'params':{
                    ...query,
