...
const portOffice =()=> {
    ...
    const [portData, setPortData] = useState([]);     // port表格数据
    const [portTotal,setPortTotal] = useState([]);//port 数量
            ...
            render:(text,record,index) => {
                return <div className='operation' >
                    ...
                    <Tooltip title={<FormattedMessage id='btn.delete' />}>
                        <a><CosButton auth='AFCM-BASE-CMS-004-B03' onClick={() => {deleteBtn(record, index)}} ><CloseCircleOutlined style={{color:'red',fontSize: '15px'}}/></CosButton> </a>
                    </Tooltip>&nbsp;
                    {/* 编辑 */}
                    ...
                    <Tooltip title={<FormattedMessage id='btn.edit' />}>
                        <a><CosButton auth='AFCM-BASE-CMS-004-B04' onClick={() => {addView(record, false)}} ><FormOutlined style={{color:'#1890ff',fontSize: '15px'}}/></CosButton> </a>
                    </Tooltip>&nbsp;
                    {/* 查看明细 */}
    ...
    const pageChange = async (pagination,search) =>{
        ...
        if(pagination.pageSize!=portPage.pageSize){
            pagination.current=1
        }
            const result = await request($apiUrl.BASE_PORT_OFFICE_SEARCH_LIST,{
                ...
                data:{
                    ...
                    params: {
                        ...
                        officeCode: queryData.portCode,    //officeCode若后续参数名称需调整再修改
            ...
            if(result.success){
                ...
                setPortPage({...pagination})
                setPortTotal(data.totalCount)
                if(datas==null){
                    setPortData([])
        ...
        const confirmModal = confirm({
            ...
            async onOk() {
                ...
                const deleteData = await request($apiUrl.BASE_PORT_OFFICE_DELETE_UUID,{
                    ...
                    data:{
                        params:{
                            officeCode: record.portCode,    //officeCode若后续参数名称需调整再修改
    ...
    const addBtn = async() => {
        ...
        setSaveType('NEW')
        setWriteFlag(false);
        setTxt(intl.formatMessage({id: 'menu.afcm.base.portAgent'})); 
        queryForm.setFieldsValue({
            popData: {
                shipownerCompanyCode: company.companyType == 0 ? company.companyCode : acquireData.defaultValue,
                hierarchyType: null,
    ...
    const handleSave = async() => {
        ...
        if(saveType=='NEW'){
            ...
            paramList = [
                ...
                {  //旧数据
                    ...
                    officeCode: recordData.portCode,
        ...
        if(!queryData.shipownerCompanyCode || !queryData.hierarchyType || !queryData.companyCode ||
            !queryData.activeDate || !queryData.sapId  || !queryData.parentSapId || !queryData.portCode){
            setInfoTips({alertStatus: 'alert-error', message: intl.formatMessage({id: 'lbl.port-office-save'})});
           return;
        }else if(queryData.portCode.length>3){
    ...
    const downloadBtn = async() => {
        ...
        const result = await request($apiUrl.BASE_PORT_OFFICE_DOWNLOAD,{
            method:"POST",
            data:{
                excelFileName: intl.formatMessage({id: 'lbl.port-download-title'}), //文件名
                params: {
                    shipownerCompanyCode: queryData.shipownerCompanyCode,
                    hierarchyType: queryData.hierarchyType,
                    officeCode: queryData.portCode,   //officeCode若后续参数名称需调整再修改
                ...
                sheetList: [{//sheetList列表
                    ...
                    sheetName: intl.formatMessage({id: 'lbl.port-download-title'}),//sheet名称
                }],
            },
            responseType: 'blob',
            headers: {
            ...
            return
                ...
                if (window.navigator.msSaveOrOpenBlob) { //msSaveOrOpenBlob 提供保存和打开按钮
                    setSpinflag(false);
                    navigator.msSaveBlob(blob, intl.formatMessage({id: 'lbl.port-download-title'}))  //msSaveBlob 只提供一个保存按钮 存储以本地方式保存文件
                } else {
                    setSpinflag(false);
                    let downloadElement = document.createElement('a');  //创建元素节点
                    let href = window.URL.createObjectURL(blob); // 创建下载的链接
                    downloadElement.href = href;
                    downloadElement.download = intl.formatMessage({id: 'lbl.port-download-title'}) + '.xlsx'; // 下载后文件名
        ...
        <div className='parent-box'>
            <div className="header-from">
                <Form onFinish={handleQuery} form={queryForm} name='search'>
                    <Row>
                        ...
                        {/* Office code */}
						<InputText span={6} name='portCode' label={<FormattedMessage id='lbl.port'/>}  span={6} /> 
                        {/* 层次类型 HRCHY_TYPE */}
            ...
            <div className="main-button">
                <div className="button-left">
					{/* 新增 */}
                    <CosButton onClick={addBtn} auth='AFCM-BASE-CMS-004-B01'><FileAddOutlined /><FormattedMessage id='lbl.new-btn'/></CosButton>
					{/* 下载 */}
                ...
                <div className="button-right">
                    ...
                    <CosButton onClick={() => pageChange(portPage,'search')}><SearchOutlined /><FormattedMessage id='btn.search' /></CosButton> 
                </div>
            </div>
            {/* ================================================================================================================== */}
            <div className="footer-table">
                ...
                <div style={{width: '90%'}}>
                    <PaginationTable
                        ...
                        pageSize={portPage.pageSize}
                        current={portPage.current}
                ...
                <div className='modalContent' style={{minWidth: '300px'}}>
                        <Form form={queryForm} name='add' onFinish={handleSave}>
                            <Row>
                                ...
                                {/* Office code */}
                                <InputText span={12} name={['popData','portCode']} label={<FormattedMessage id='lbl.port'/>}  isSpan={true} maxLength={3} disabled={writeFlag}/> 
                                {/* 层次类型 HRCHY_TYPE */}
                        ...
                        <div className='main-button'>
                            ...
                            <div className='button-right'>
                                {/* 保存 */}
                                <CosButton  onClick={() => handleSave()}  disabled={writeFlag} auth='AFCM-BASE-CMS-004-B02'><FormattedMessage id='lbl.save' /></CosButton>
                                {/* 取消 */}
