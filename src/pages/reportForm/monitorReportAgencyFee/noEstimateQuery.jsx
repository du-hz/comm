...
const noEstimateQuery =()=> {
    ...
    useEffect(()=>{
        ...
        acquireSelectData('AFCM.FEETYPE', setSubclass, $apiUrl);  // 费用小类 
    ...
    const downloadBtn =async () => {
        ...
        if(!queryData.activityDate && !queryData.svvdId ){
            Toast('',intl.formatMessage({id:'lbl.Report-agfee-estime'}), 'alert-error', 5000, false)
            setBackFlag(false);
            return;
        }
        setLoading(true);
        ...
        const result = await request($apiUrl.REPORT_FORM_AGENCY_FEE_NO_BILLING_DOWNLOAD,{
            ...
            data:{
                ...
                operateType: "AFCM-AG-MR-001",
                excelFileName: intl.formatMessage({id: 'menu.afcm.reportForm.agFee.noEstimateQuery'}), //文件名
                sheetList: [
                    {//sheetList列表
                        dataCol: {//列表字段
        ...
        if(result.size<=0){  //若无数据，则不下载
            ...
            if (window.navigator.msSaveOrOpenBlob) { //msSaveOrOpenBlob 提供保存和打开按钮
                navigator.msSaveBlob(blob, intl.formatMessage({id: 'menu.afcm.reportForm.agFee.noEstimateQuery'}))  //msSaveBlob 只提供一个保存按钮 存储以本地方式保存文件
            } else {
                let downloadElement = document.createElement('a');  //创建元素节点
                let href = window.URL.createObjectURL(blob); // 创建下载的链接
                downloadElement.href = href;
                downloadElement.download = intl.formatMessage({id: 'menu.afcm.reportForm.agFee.noEstimateQuery'})+ '.xlsx'; // 下载后文件名
    ...
    const pageChange = async () =>{
        ...
        if(result.success){
            setLoading(false);
            // let cntrList=result.data.cntrList
            ...
            <div className="header-from">
                <Form form={queryForm} name='search' onFinish={handleQuery}>
                    <Row>
                        ...
                        {/* SVVD */}
                        <InputText name='svvdId' label={<FormattedMessage id='lbl.SVVD'/>} span={6} styleFlag={backFlag} formlayouts={formlayouts}/> 
                        {/* 港口 */}
                        <InputText name='portCode' label={<FormattedMessage id='lbl.port'/>} span={6} formlayouts={formlayouts}/> 
                        {/* 信息类别 */}
                        <Select name='category' label={<FormattedMessage id='lbl.Info-category'/>} span={6} flag={true} options={infoCategory.values} formlayouts={formlayouts}/>
                    </Row>
                </Form>
...
export default noEstimateQuery