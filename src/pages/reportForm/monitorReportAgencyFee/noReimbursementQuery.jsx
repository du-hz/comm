...
const noReimbursementQuery =()=> {
    ...
    const downloadBtn =async () => {
        ...
        if(!queryData.activityDate && !queryData.svvdId && !queryData.yfListCode){
            Toast('',intl.formatMessage({id:'lbl.Report-agfee-reimbur'}), 'alert-error', 5000, false)
            setBackFlag(false);
            return;
        }
        setLoading(true)
        ...
        const result = await request($apiUrl.REPORT_FORM_AGENCY_FEE_NO_BILLING_DOWNLOAD,{
            ...
            data:{
                ...
                operateType: "AFCM-AG-MR-003",
                excelFileName: intl.formatMessage({id: 'menu.afcm.reportForm.agFee.noReimburQuery'}), //文件名
                sheetList: [
                    {//sheetList列表
                        dataCol: {//列表字段
        ...
        if(result.size<=0){  //若无数据，则不下载
            ...
            if (window.navigator.msSaveOrOpenBlob) { //msSaveOrOpenBlob 提供保存和打开按钮
                navigator.msSaveBlob(blob, intl.formatMessage({id: 'menu.afcm.reportForm.agFee.noReimburQuery'}))  //msSaveBlob 只提供一个保存按钮 存储以本地方式保存文件
            } else {
                let downloadElement = document.createElement('a');  //创建元素节点
                let href = window.URL.createObjectURL(blob); // 创建下载的链接
                downloadElement.href = href;
                downloadElement.download = intl.formatMessage({id: 'menu.afcm.reportForm.agFee.noReimburQuery'})+ '.xlsx'; // 下载后文件名
            ...
            <div className="header-from">
                <Form form={queryForm} name='search' onFinish={handleQuery}>
                    <Row>
                        ...
                        {/* 发票号码 */}
                        <InputText name='yfListCode' label={<FormattedMessage id='lbl.Invoice-number'/>} styleFlag={backFlag} span={6} formlayouts={formlayouts}/> 
                        {/* 信息类别 */}
