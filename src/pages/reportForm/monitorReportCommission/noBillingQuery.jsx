...
const noBillingQuery =()=> {
    ...
    const downloadBtn =async () => {
        ...
        if(!queryData.erReceiptCode && !queryData.activityDate && !queryData.billReferenceCode){
            Toast('',intl.formatMessage({id:'lbl.Report-form-bill'}), 'alert-error', 5000, false)
            setBackFlag(false);
            return;
        }
        setSpinflag(true);
        ...
        const result = await request($apiUrl.REPORT_FORM_COMM_NO_BILLING_DOWNLOAD,{
            ...
            data:{
                ...
                operateType: "AFCM-COMM-MR-002",
                excelFileName: intl.formatMessage({id: 'menu.afcm.reportForm.comm.noBillQuery'}), //文件名
                sheetList: [
                    {//sheetList列表
                        dataCol: {//列表字段
        ...
        if(result.size<=0){  //若无数据，则不下载
            ...
            if (window.navigator.msSaveOrOpenBlob) { //msSaveOrOpenBlob 提供保存和打开按钮
                navigator.msSaveBlob(blob, intl.formatMessage({id: 'menu.afcm.reportForm.comm.noBillQuery'}))  //msSaveBlob 只提供一个保存按钮 存储以本地方式保存文件
            } else {
                let downloadElement = document.createElement('a');  //创建元素节点
                let href = window.URL.createObjectURL(blob); // 创建下载的链接
                downloadElement.href = href;
                downloadElement.download = intl.formatMessage({id: 'menu.afcm.reportForm.comm.noBillQuery'})+ '.xlsx'; // 下载后文件名
        ...
        const result = await request($apiUrl.REPORT_FORM_COMM_NO_BILLING_SEARCH_LIST,{
            ...
            data:{
                "params": {
                    ...
                    erReceiptCode: queryData.erReceiptCode,
            ...
            <div className="header-from">
                <Form form={queryForm} name='search' onFinish={handleQuery}>
                    <Row>
                        ...
                        {/* 预估单号码 */}
                        <InputText name='erReceiptCode' styleFlag={backFlag} label={<FormattedMessage id='lbl.Estimated-order-number'/>} span={6} formlayouts={formlayouts}/> 
                        {/* 信息类别 */}
