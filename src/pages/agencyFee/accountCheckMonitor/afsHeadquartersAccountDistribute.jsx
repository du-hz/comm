...
const afsAccountDistribute =()=> {
    ...
    const columns=[
        ...
        {
            title: <FormattedMessage id='lbl.subject-belong'/>,//科目所属
            dataIndex: 'newkoType',
            sorter: false,
            width: 80,
            align:'left', 
        ...
        {
            title: <FormattedMessage id='lbl.Bookkeeping-amount-month'/>,//当月累计记账金额
        ...
        {
            title: <FormattedMessage id='lbl.No-bookkeeping-amount-month'/>,//非当月累计记账金额
        ...
        {
            title: <FormattedMessage id='lbl.Bookkeeping-amount-year'/>,//当年累计记账金额
    ...
    const pageChange = async (pagination,search) =>{
        ...
        const result = await request($apiUrl.AGENCY_FEE_ACCOUNT_CHECK_MONITOR_SEARCH_LIST,{
            ...
            data:{
                "page": pagination,
                "params": {
                    "entryCode": "AFCM_LEDGER_AFS_COST",
                    "paramEntity":{
    ...
    const downloadBtn = async() => {
        ...
        const result = await request($apiUrl.BASE_COMM_CONFIG_DOWNLOAD,{
            ...
            data:{
                ...
                excelFileName: intl.formatMessage({id: 'menu.afcm.agfee-stl.checkMonitor.afsRecord'}), //文件名
                sheetList: [{//sheetList列表
                    dataCol: {//列表字段
                        ...
                        newkoType: intl.formatMessage({id: "lbl.subject-belong"}),
                    ...
                    sumCol: {},//汇总字段
                    sheetName: intl.formatMessage({id: 'menu.afcm.agfee-stl.checkMonitor.afsRecord'}),//sheet名称
                }],
            },
            responseType: 'blob',
            headers: {
        ...
        if(result.size<1){  //若无数据，则不下载
            ...
            if (window.navigator.msSaveOrOpenBlob) { //msSaveOrOpenBlob 提供保存和打开按钮
                setSpinflag(false);
                navigator.msSaveBlob(blob, intl.formatMessage({id: 'menu.afcm.agfee-stl.checkMonitor.afsRecord'}))  //msSaveBlob 只提供一个保存按钮 存储以本地方式保存文件
            } else {
                setSpinflag(false);
                let downloadElement = document.createElement('a');  //创建元素节点
                let href = window.URL.createObjectURL(blob); // 创建下载的链接
                downloadElement.href = href;
                downloadElement.download = intl.formatMessage({id: 'menu.afcm.agfee-stl.checkMonitor.afsRecord'})+ '.xlsx'; // 下载后文件名
...
export default afsAccountDistribute