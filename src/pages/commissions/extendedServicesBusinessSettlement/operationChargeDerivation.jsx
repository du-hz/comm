...
import { momentFormat, acquireSelectData, agencyCodeData, acquireSelectDataExtend} from '@/utils/commonDataInterface';
...
const searchPreAgreementMailFeeAgmtList =()=> {
    ...
    const [cmbVatFlag,setCmbVatFlag] = useState({});//是否增值税
    ...
    useEffect(() => {   // 默认值
        queryForm.setFieldsValue({
            ...
            cmbVatFlag:cmbVatFlag.defaultValue
        })
    }, [company, acquireData])

    useEffect(() => {
        agencyCodeData($apiUrl,setAgencyCode,setCompany)//代理编码
        acquireSelectData('VAT.FLAG.EXT', setCmbVatFlag, $apiUrl);// 是否增值税
    ...
    const columns=[
        ...
        {
            title:<FormattedMessage id="lbl.argue.sapId" /> ,//客户编码
    ...
    const pageChange= async(pagination,options,search) =>{
        ...
        let localsearch= await request($apiUrl.COMM_EXTENSION_QUERYCR_EXTENSION,{
            method:"POST",
            data:{
                "page": pagination,
                "params":{
        ...
        if(localsearch.success){
            ...
            let datas=data ? localsearch.data.resultList : null
     ...
     const downlod = async () =>{
        Toast('','', '', 5000, false)
        let query = queryForm.getFieldsValue()
        let downData = await request($apiUrl.COMM_EXTENSION_EXP_EXTENSION,{
            method:"POST",
            data:{
                "params":{
                    'shipownerCompanyCode': query.shipownerCompanyCode,
                ...
                'excelFileName':formatMessage({id:'menu.afcm.comm.ext-stl.ope-chrg-der'}),
                sheetList: [
                    {//sheetList列表
                        dataCol: {//列表字段
                            ...
                            generateDate: formatMessage({id:"lbl.generated-time" }),
                            ...
                            verifyStatus: formatMessage({id:"lbl.state" }),
                            profitCenterCode:formatMessage({id:'profit-center'}),
                            customerSapId: formatMessage({id:"lbl.argue.sapId" }),
                    ...
                    'sheetName':formatMessage({id:'menu.afcm.comm.ext-stl.ope-chrg-der'}),
                },
            ]
            },
            responseType: 'blob',
        ...
        if(downData.size<1){
            ...
            if (window.navigator.msSaveOrOpenBlob) { //msSaveOrOpenBlob 提供保存和打开按钮
                navigator.msSaveBlob(blob, formatMessage({id: 'menu.afcm.comm.ext-stl.ope-chrg-der'}))  //msSaveBlob 只提供一个保存按钮 存储以本地方式保存文件
            } else {
                let downloadElement = document.createElement('a');  //创建元素节点
                let href = window.URL.createObjectURL(blob); // 创建下载的链接
                console.log(href)
                downloadElement.href = href;
                downloadElement.download = formatMessage({id: 'menu.afcm.comm.ext-stl.ope-chrg-der'}) + '.xlsx'; // 下载后文件名
            ...
            <div className='header-from'>
                ...
                >
                    <Row>
                        ...
                        {/* 是否增值税 */}
                        <SelectVal name='cmbVatFlag' label={<FormattedMessage id='lbl.VAT-or-not'/>} span={6} options={cmbVatFlag.values}/>
                        {/* 提单号码 */}
